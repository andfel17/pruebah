...
import HentaiItemSearch from './HentaiItemSearch';
...
export default class Search extends Component {
    ...
    fetchHentaisApi = (q) => {
        ...
        axios.get(`${process.env.APIURL}hentai/search?search=${q}`,{
    ...
    SearchContainer = () => {
        const { hentais, query, fetching } = this.state;
        return (
            <div className={styles.searchContainer}>
                ...
                <div className={styles.results}>
                    { fetching 
                        ...
                        :   
                            <>  { hentais.length > 0 && (
                                    hentais?.map((hentai, i) => {
                                        return(
                                            <HentaiItemSearch key={i} data={hentai}/>
                                        )   
                                    })
                                )}
                                <div className={styles.empty}>{ !query || query?.length < 3 ? `Min. 3 caracteres` : ( hentais.length > 0 ? `Resultados para ${query}` : `No hay resultados`) }</div>
                            </>
                    }
                </div>
            </div>
