...
import { filterHentai } from '../../helpers/Functions';
...
class index extends Component {
                ...
                <div className={styles.ListTypes}>
                    <div className={styles.type}>
                        ...
                        <select name={"status"} value={filter?.status} id={"status"} onChange={this.handleChange}>
                            ...
                            {filterHentai()?.status?.map((item, idx) => (
                    ...
                    <div className={styles.type}>
                        ...
                        <select name={"year"} value={filter?.year} id={"year"} onChange={this.handleChange}>
                            <option value="">Todos</option>
                            {filterHentai()?.years?.map((item, idx) => (
                    ...
                    <div className={styles.type}>
                        ...
                        <select name={"genre"} value={filter?.genre} id={"genre"} onChange={this.handleChange}>
                            <option value="">Todos</option>
                            {filterHentai()?.genres?.map((item, idx) => (
    ...
    render() {
        ...
        return (
            <Layout>
                <Head>
                    <title>{`Lista de hentais • ${process.env.NAME}`}</title>
                    <meta name="description" content={`Hentai Online Gratis, mira los últimos capitulos de los hentais del momento sin ninguna restriccion subtitulados al español latino en ${process.env.NAME}`} />
                    <link rel="canonical" href={`${process.env.URL}/hentais`} />
                    <meta name="og:title" content={`Lista de hentais • ${process.env.NAME}`} />
                    <meta name="og:description" content={`Hentai Online Gratis, mira los últimos capitulos de los hentais del momento sin ninguna restriccion subtitulados al español latino en ${process.env.NAME}`} />
                ...
                <main className={styles.container}>
                    <ListHentais paginate={this.paginationHentais()} filters={this.filterHentais()} hentais={data?.data}/>
                </main>
            </Layout>
        );
    }
...
index.getInitialProps = async (context) => {
    ...
    const res = await api.get(`hentai/list?${queryString({ ...context?.query })}`);
	return { 
        data: res.data
    }
}
